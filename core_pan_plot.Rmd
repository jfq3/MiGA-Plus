...
```{r}
a.core <- a %>% 
  dplyr::select(genomes, core_avg, core_q1, core_q3) %>% 
  dplyr::mutate(Genome = "Core") %>% 
  dplyr::rename(avg = core_avg,
                q1 = core_q1,
                q3 = core_q3)

a.pan <- a %>% 
  dplyr::select(genomes, pan_avg, pan_q1, pan_q3) %>% 
  dplyr::mutate(Genome = "Pan") %>% 
  dplyr::rename(avg = pan_avg,
                q1 = pan_q1,
                q3 = pan_q3)
```
...
ggplot() +
  ...
  geom_line(data = dplyr::filter(df, Genome == "Pan"),
            aes(x=genomes, y=avg), color = "black", linetype = "dotdash") +
